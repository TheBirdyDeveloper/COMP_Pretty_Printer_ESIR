...
class WhGenerator extends AbstractGenerator {
	...
	def void doGenerate(Resource resource, IFileSystemAccess2 fsa, IGeneratorContext context, String outputName, int ifIndent, int commandIndent, int whileIndent, int forIndent) {
		...
		this.symTable = new Hashtable<String, Integer>;
		this.cptFunctions = 0;
		this.cptInput = 0;
		this.cptOutput = 0;
	...
	def void symTable(Program p){
		
		if(!(symTable.containsKey(p.name))){
			this.symTable.put((p.name),cptFunctions)
			cptFunctions++;
			if((p.definition.input.vars.size) >= 1){
				for(i:0..(p.definition.input.vars).size-1){
					//if(!(symTable.containsKey(p.name))){
						this.symTable.put(p.definition.input.vars.get(i),cptInput)
						cptInput++
					//}
				}
			}
